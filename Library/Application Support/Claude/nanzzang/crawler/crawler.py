...
def generate_topics_with_ai(headlines: list[str], count: int = 5) -> list[dict]:
...
카테고리는 반드시 다음 중 하나: politics, economy, social, culture, daily, love, work, story
...
[
  {{
    ...
    print(f"[AI] {len(topics)}개 토픽 생성 완료")
    return topics
...
def run(count: int = 5, dry_run: bool = False):
    ...
    # 2. AI 토픽 생성
    print(f"\n[2단계] Claude AI로 토픽 {count}개 생성 중...")
    topics = generate_topics_with_ai(headlines, count)
